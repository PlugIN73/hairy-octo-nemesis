function DeviceG5(node, width, height) {
	DeviceG5.superclass.constructor.call(this, node, width, height);
	this.state = {
	...
	this.state_time_variables = ["tact", "signal"]; 			// массив названий переменных состояния прибора, котоорые зависят от времени, а не от действий пользователя. Троицкий
	this.InitEx(); 												// загрузить упражнения из файла g5 exercises.js
	this.name = "g5";											// Название папки для поиска изображений
	this.title = 'Генератор импульсов точной амплитуды Г5-75';
}

extend(DeviceG5, Map);

DeviceG5.prototype.getArea = function () {
...
DeviceG5.prototype.getAreaPre = function () {
	var area = [];
...
DeviceG5.prototype.definitionControl = function () {
	var c = []
